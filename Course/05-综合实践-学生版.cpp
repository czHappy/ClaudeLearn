// 05-综合实践-学生版.cpp
// 魔法学院战斗竞技场 - 学生实践版
// 请在标记了 TODO 的地方补充代码

#include <iostream>
#include <string>
#include <cstdlib>
#include <ctime>
using namespace std;

// ============================================
// Character 基类
// ============================================
class Character {
protected:
    string name;
    int hp;
    int maxHp;
    int attackPower;
    string school;

public:
    // 构造函数
    Character(string n, int health, int attack, string sch)
        : name(n), hp(health), maxHp(health), attackPower(attack), school(sch) {
    }

    // 纯虚函数 - 子类必须实现
    virtual void attack(Character* target) = 0;

    // 受到伤害
    void takeDamage(int damage) {
        hp -= damage;
        if (hp < 0) hp = 0;
        cout << name << " 受到 " << damage << " 点伤害！";
        if (hp == 0) {
            cout << " (已阵亡)";
        }
        cout << endl;
    }

    // 判断是否存活
    bool isAlive() {
        return hp > 0;
    }

    // 显示状态（生命值进度条）
    void showStatus() {
        cout << "[" << school << "] " << name << " ";

        // 计算进度条
        int filled = (hp * 10) / maxHp;
        cout << "[";
        for (int i = 0; i < 10; i++) {
            if (i < filled) {
                cout << "█";
            } else {
                cout << "░";
            }
        }
        cout << "] " << hp << "/" << maxHp << " HP" << endl;
    }

    // 显示技能信息
    virtual void showSkillInfo() {
        cout << "[" << school << "] " << name << " 的技能信息" << endl;
    }

    // 虚析构函数
    virtual ~Character() {}

    // 辅助方法
    string getName() const { return name; }
    string getSchool() const { return school; }
};

// ============================================
// Warrior 战士类 - 请完成此类
// ============================================
class Warrior : public Character {
private:
    int rage;  // 怒气值 (0-100)

public:
    // TODO: 实现构造函数
    Warrior(string name, int hp, int attack)
        : Character(name, hp, attack, "战士") {
        // 初始化 rage
    }

    // TODO: 重写 attack 方法
    void attack(Character* target) override {
        cout << "\n" << name << " 发起攻击：" << endl;

        // TODO: 判断怒气是否满100
        // 如果满100：
        //   - 造成 attackPower * 3 伤害
        //   - 输出狂暴文案："💥 [狂暴打击] 怒气爆发！造成 X 点毁灭性伤害！"
        //   - 清空怒气
        // 如果未满：
        //   - 造成 attackPower 伤害
        //   - 输出普通文案："[战士] 挥舞巨剑，造成 X 点物理伤害！"
        //   - 怒气 +20

        // 显示怒气值
        cout << "怒气值: " << rage << "/100" << endl;
    }

    void showSkillInfo() override {
        cout << "【战士技能】" << endl;
        cout << "- 狂暴打击：怒气满100时，下次攻击造成3倍伤害" << endl;
        cout << "- 当前怒气：" << rage << "/100" << endl;
    }
};

// ============================================
// Mage 法师类 - 请完成此类
// ============================================
class Mage : public Character {
private:
    int mp;     // 当前魔法值
    int maxMp;  // 最大魔法值

public:
    // TODO: 实现构造函数
    Mage(string name, int hp, int attack)
        : Character(name, hp, attack, "法师") {
        // 初始化 mp 和 maxMp
    }

    // TODO: 重写 attack 方法
    void attack(Character* target) override {
        cout << "\n" << name << " 发起攻击：" << endl;

        // TODO: 判断魔法值是否足够
        // 如果 mp >= 20：
        //   - 消耗 20 MP
        //   - 造成 attackPower * 1.5 伤害
        //   - 输出："✨ [法师] 释放火球术，造成 X 点魔法伤害！"
        // 如果 mp < 20：
        //   - 造成 attackPower * 0.5 伤害
        //   - 输出："[法师] 魔力耗尽，用法杖敲击，造成 X 点伤害..."

        // 显示魔法值
        cout << "剩余魔力: " << mp << "/" << maxMp << " MP" << endl;
    }

    // TODO: 实现魔力恢复方法
    void restoreMana() {
        // 恢复 10 MP，不超过 maxMp
    }

    void showSkillInfo() override {
        cout << "【法师技能】" << endl;
        cout << "- 火球术：消耗20MP，造成1.5倍伤害" << endl;
        cout << "- 当前魔力：" << mp << "/" << maxMp << " MP" << endl;
    }
};

// ============================================
// Archer 游侠类 - 请完成此类
// ============================================
class Archer : public Character {
private:
    int arrows;  // 箭矢数量

public:
    // TODO: 实现构造函数
    Archer(string name, int hp, int attack)
        : Character(name, hp, attack, "游侠") {
        // 初始化 arrows（初始10支）
    }

    // TODO: 重写 attack 方法
    void attack(Character* target) override {
        cout << "\n" << name << " 发起攻击：" << endl;

        // TODO: 判断是否有箭矢
        // 如果有箭矢：
        //   - 消耗 1 支箭矢
        //   - 30% 几率触发连射（使用 rand() % 100 < 30 判断）
        //   - 连射：造成 attackPower * 2 伤害，输出："🎯 [连珠箭] 快速连射！造成 X 点伤害！"
        //   - 普通：造成 attackPower 伤害，输出："🏹 [游侠] 精准射击，造成 X 点伤害！"
        //   - 显示剩余箭矢
        // 如果无箭矢：
        //   - 造成 attackPower * 0.3 伤害
        //   - 输出："[游侠] 箭矢用尽，匕首攻击，造成 X 点伤害..."
    }

    void showSkillInfo() override {
        cout << "【游侠技能】" << endl;
        cout << "- 精准射击：30%几率连射造成双倍伤害" << endl;
        cout << "- 剩余箭矢：" << arrows << endl;
    }
};

// ============================================
// Assassin 刺客类 - 挑战任务（可选）
// ============================================
class Assassin : public Character {
private:
    int energy;        // 能量值
    bool stealthMode;  // 隐身状态

public:
    // TODO: 实现构造函数（挑战）
    Assassin(string name, int hp, int attack)
        : Character(name, hp, attack, "刺客") {
        // 初始化 energy（初始50） 和 stealthMode（false）
    }

    // TODO: 重写 attack 方法（挑战）
    void attack(Character* target) override {
        cout << "\n" << name << " 发起攻击：" << endl;

        // TODO: 实现刺客的攻击逻辑
        // 1. 如果处于隐身状态：
        //    - 造成 attackPower * 5 伤害
        //    - 输出："💀 [暗影暗杀] 从阴影中突袭！造成 X 点致命伤害！！！"
        //    - 解除隐身状态
        // 2. 如果有足够能量（>= 30）且未隐身：
        //    - 可以选择进入隐身（这里简化为自动进入）
        //    - 消耗 30 能量
        //    - 进入隐身状态
        //    - 输出提示
        // 3. 普通攻击：
        //    - 50% 几率暴击（2倍伤害）
        //    - 暴击："⚡ [暴击] 致命一击！造成 X 点伤害！"
        //    - 普通："🗡️ [刺客] 利刃攻击，造成 X 点伤害！"
    }

    // TODO: 实现能量恢复方法
    void restoreEnergy() {
        // 恢复 15 能量，不超过 100
    }

    void showSkillInfo() override {
        cout << "【刺客技能】" << endl;
        cout << "- 暗影暗杀：消耗30能量，下次攻击造成5倍伤害" << endl;
        cout << "- 暴击：50%几率造成2倍伤害" << endl;
        cout << "- 当前能量：" << energy << "/100" << endl;
    }
};

// ============================================
// Arena 竞技场类 - 请完成此类
// ============================================
class Arena {
public:
    // TODO: 实现 battle 方法
    void battle(Character* fighter1, Character* fighter2) {
        // 显示战斗开始信息
        cout << "\n╔══════════════════════════════╗" << endl;
        cout << "║        战斗开始！             ║" << endl;
        cout << "╠══════════════════════════════╣" << endl;
        cout << "║  " << fighter1->getName() << " VS " << fighter2->getName() << endl;
        cout << "╚══════════════════════════════╝" << endl;

        // TODO: 显示双方初始状态
        cout << "\n初始状态：" << endl;
        // 调用 showStatus() 方法

        int round = 1;

        // TODO: 回合制战斗循环
        while (/* 双方都还活着 */) {
            cout << "\n======== 第 " << round << " 回合 ========" << endl;

            // TODO: fighter1 攻击 fighter2


            // TODO: 显示战况
            cout << "\n--- 战况 ---" << endl;
            // 显示双方状态

            // TODO: 判断 fighter2 是否存活
            // 如果不存活，跳出循环

            // TODO: fighter2 反击


            // TODO: 显示战况


            // TODO: 判断 fighter1 是否存活
            // 如果不存活，跳出循环

            // TODO: 回合结束处理
            // - 法师恢复魔力
            // - 刺客恢复能量

            round++;
        }

        // TODO: 显示战斗结果
        cout << "\n╔══════════════════════════════╗" << endl;
        cout << "║        战斗结束！             ║" << endl;
        cout << "╠══════════════════════════════╣" << endl;
        // 显示获胜者
        cout << "╚══════════════════════════════╝" << endl;
    }

private:
    // 辅助方法：显示分隔线
    void showSeparator() {
        cout << "━━━━━━━━━━━━━━━━━━━━━━━━━━" << endl;
    }
};

// ============================================
// 主函数 - 请完成测试代码
// ============================================
int main() {
    // 初始化随机数种子
    srand(time(0));

    cout << "╔═══════════════════════════════════════════════╗" << endl;
    cout << "║                                               ║" << endl;
    cout << "║      🎮 魔法学院战斗竞技场                    ║" << endl;
    cout << "║      霍格沃兹年度大赛                         ║" << endl;
    cout << "║                                               ║" << endl;
    cout << "╚═══════════════════════════════════════════════╝" << endl;

    // TODO: 创建角色（使用基类指针，体现多态）
    // Character* warrior = new Warrior("勇士阿瑟", 120, 50);
    // Character* mage = ...
    // Character* archer = ...
    // Character* assassin = ...

    // TODO: 创建竞技场
    // Arena arena;

    // TODO: 进行战斗测试
    // 战斗1：战士 vs 法师
    // cout << "\n【第一场战斗】" << endl;
    // arena.battle(warrior, mage);

    // 战斗2：游侠 vs 刺客
    // ...

    // 战斗3：获胜者对决
    // ...

    // TODO: 释放内存
    // delete warrior;
    // delete mage;
    // ...

    return 0;
}

/*
 * ============================================
 * 实现提示
 * ============================================
 *
 * 1. 生命值进度条示例：
 *    [████████░░] 80/100 HP
 *    计算方法：filled = (hp * 10) / maxHp
 *
 * 2. 随机数判断示例：
 *    if (rand() % 100 < 30) {
 *        // 30% 几率触发
 *    }
 *
 * 3. 多态使用示例：
 *    Character* hero = new Warrior("阿瑟", 100, 50);
 *    hero->attack(enemy);  // 自动调用 Warrior 的 attack
 *
 * 4. 类型转换（如果需要）：
 *    Mage* mage = dynamic_cast<Mage*>(character);
 *    if (mage) {
 *        mage->restoreMana();
 *    }
 *
 * ============================================
 * 编译和运行
 * ============================================
 *
 * g++ -std=c++11 05-综合实践-学生版.cpp -o battle
 * ./battle
 *
 * ============================================
 * 评分要点
 * ============================================
 *
 * 1. 正确使用继承和多态 (30分)
 * 2. 所有角色类实现正确 (40分)
 * 3. Arena 战斗逻辑完整 (20分)
 * 4. 测试充分，内存管理正确 (10分)
 *
 * 加分项：
 * - 输出格式美观
 * - 代码注释清晰
 * - 实现了刺客类（挑战）
 */